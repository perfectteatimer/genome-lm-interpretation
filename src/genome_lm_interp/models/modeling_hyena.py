...
class HyenaDNAForTokenClassification(HyenaDNAPreTrainedModel):
    ...
    def __init__(self, config, **kwargs):
        ...
        # Class weights for the (heavily imbalanced) token-classification loss.
        # Defaults to [0.7, 2.0] (the best Z-DNA setup); override via
        # ``config.class_weights`` or the ``class_weights`` keyword argument.
        class_weights = kwargs.get(
            "class_weights", getattr(config, "class_weights", [0.7, 2.0])
        )
        weight = (
        ...
        self.criterion = nn.CrossEntropyLoss(ignore_index=-100, weight=weight)
